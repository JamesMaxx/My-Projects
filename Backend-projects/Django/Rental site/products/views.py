...
from .forms import OrderForm, CustomerForm, ReviewForm
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Calculate totals
            subtotal = sum(item.total_price for item in cart_items)
            shipping_cost = form.cleaned_data.get('shipping_cost', Decimal('15.00'))
            total = subtotal + shipping_cost
        ...
        # Pre-fill with customer address
        initial_data = {
            'shipping_address': customer.shipping_address,
            'billing_address': customer.billing_address or customer.shipping_address,
        }
